...
import com.deefrent.rnd.fieldapp.network.models.IncompleteRegResponse
...
class IncompleteRegDashboardViewModel : ViewModel() {
    ...
    val responseStatus: LiveData<GeneralResponseStatus>
        get() = _responseStatus
   private var _incompleteData = MutableLiveData<IncompleteRegResponse>()
    val incompleteData:LiveData<IncompleteRegResponse>
        get() = _incompleteData
    ...
    fun stopObserving() {
     ...
     fun getIncompleteData() {
        uiScope.launch {
            _responseStatus.value= GeneralResponseStatus.LOADING
            val request = FieldAgentApi.retrofitService.getIncompleteRegistrationAsync()
            try {
